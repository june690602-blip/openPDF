...
- **홈 화면 (런처, 2026-06-09)** — 앱 실행 = `HomeActivity`(런처): **PDF 열기**(SAF) + **최근 파일** 목록(탭=열기, 길게=삭제) + 정보·라이선스. 파일 선택 → 뷰어(`MainActivity`)로 전환(SAF=VIEW+data, 최근=`EXTRA_LOCAL_PATH`). 카톡 VIEW/SEND는 manifest 필터로 뷰어 직행(홈 거치지 않음). `MainActivity`는 인텐트/EXTRA 없이 직접 실행 시 번들 샘플 자동 오픈(테스트·개발용 폴백). (opendwg 홈 패턴.)
- **출시(Phase 7)** — `LICENSE`(AGPL v3 전문) + 앱 내 **정보·라이선스** 고지(소스 링크 tappable + MuPDF/hwplib attribution) + `README`. GitHub 공개 레포: **github.com/june690602-blip/openPDF** (PUBLIC, AGPL-3.0).
